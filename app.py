...
st.set_page_config(page_title="Should Cost Calculation v11", layout="wide")
...
RAW_COLS = [
    ...
    "Suggested $/lb","Override $/lb","Source Tag","Source/Notes",
    "Low $/lb","Base $/lb","High $/lb"
]
if "raw_df" not in st.session_state:
    st.session_state.raw_df = pd.DataFrame([
        {"Category":"Primary","Item":"","Price ($/unit)":None,"CF Unit":"lb","CF Value (unit/lb)":None,
         "Suggested $/lb":None,"Override $/lb":None,"Source Tag":"Manual Quote","Source/Notes":"",
         "Low $/lb":None,"Base $/lb":None,"High $/lb":None}
    ], columns=RAW_COLS)

MFG_COLS = ["Category","Item","Value ($/lb)","Source Tag","Source/Notes","Low $/lb","Base $/lb","High $/lb"]
if "mfg_df" not in st.session_state:
    st.session_state.mfg_df = pd.DataFrame([
        {"Category":"Utility","Item":"Electricity","Value ($/lb)":None,"Source Tag":"Manual Quote","Source/Notes":"",
         "Low $/lb":None,"Base $/lb":None,"High $/lb":None},
        {"Category":"Conversion","Item":"Conversion costs","Value ($/lb)":None,"Source Tag":"Manual Quote","Source/Notes":"",
         "Low $/lb":None,"Base $/lb":None,"High $/lb":None},
    ], columns=MFG_COLS)

LOG_COLS = ["Item","Value ($/lb)","Source Tag","Source/Notes","Low $/lb","Base $/lb","High $/lb"]
if "log_df" not in st.session_state:
    st.session_state.log_df = pd.DataFrame([
        {"Item":"Transportation","Value ($/lb)":None,"Source Tag":"Manual Quote","Source/Notes":"",
         "Low $/lb":None,"Base $/lb":None,"High $/lb":None}
    ], columns=LOG_COLS)
...
# ===== Manufacturing (includes Utilities) =====
st.subheader("Manufacturing (includes Utilities)")
with st.form("add_mfg_form", clear_on_submit=True):
    mc1, mc2, mc3 = st.columns([1,2,1])
    with mc1:
        mfg_category = st.selectbox("Category", ["Utility","Conversion","Ops","Overhead","Other"], index=0)
    with mc2:
...
with st.form("add_log_form", clear_on_submit=True):
    ...
    ln = st.text_input("Source/Notes", key="log_source_notes", placeholder="e.g., lane calc, carrier quote")
    submitted_l = st.form_submit_button("Add row")
    if submitted_l:
        new_l = {
            "Item": log_item, "Value ($/lb)": log_value,
            "Source Tag": log_source_tag, "Source/Notes": ln,
            "Low $/lb": None, "Base $/lb": None, "High $/lb": None
        }
...
# Manufacturing subtotal = Raw + Manufacturing
...
tmc_low, tmc_base, tmc_high = ms_low, ms_base, ms_high
